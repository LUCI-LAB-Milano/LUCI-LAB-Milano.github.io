...
PLACEHOLDER_VALUES = {"", "-", "--", "n/a", "na", "none", "null", "en", "it"}
...
def clean_placeholder(value: str | None) -> str:
...
def csv_row_to_item(row: dict[str, Any], member_name: str, base_url: str) -> dict[str, Any] | None:
    ...
    abstract = clean_placeholder(first_value(row, ABSTRACT_KEYS))
    ...
    return {
        ...
        "journal_or_publisher": clean_placeholder(journal) or clean_placeholder(publisher) or "",
        "abstract": abstract,
        "members": [member_name],
    }
...
def heading_matches_abstract(tag: Tag) -> bool:
    if tag.name not in {"h1", "h2", "h3", "h4", "h5", "h6", "dt", "strong", "b"}:
        return False
    text = normalize_space(tag.get_text(" ", strip=True)).lower().strip(":")
    return text in {"abstract", "riassunto"}


def extract_abstract_from_dom(soup: BeautifulSoup) -> str:
    meta = soup.find("meta", attrs={"name": re.compile(r"description|dc\.description", re.I)})
    if meta:
        value = clean_placeholder(meta.get("content"))
        if value:
            return value

    for heading in soup.find_all(heading_matches_abstract):
        parts: list[str] = []
        for sibling in heading.next_siblings:
            ...
            if text:
                parts.append(text)
        abstract = clean_placeholder(" ".join(parts))
        if abstract:
            return abstract

    labels = soup.find_all(string=re.compile(r"^\s*(Abstract|Riassunto)\s*$", re.I))
    for label in labels:
        parent = label.parent if isinstance(label.parent, Tag) else None
        if not parent:
            continue
        container = parent.parent if isinstance(parent.parent, Tag) else parent
        text = normalize_space(container.get_text(" ", strip=True))
        text = re.sub(r"^(Abstract|Riassunto)\s*", "", text, flags=re.I)
        abstract = clean_placeholder(text)
        if abstract:
            return abstract
...
def fetch_publication_abstract(air_url: str, cache: dict[str, str], warnings: list[str]) -> str:
    if not air_url:
        return ""
    normalized_url = clean_url(air_url) or air_url
    if normalized_url in cache:
        return cache[normalized_url]
    if "/handle/" not in normalized_url:
        cache[normalized_url] = ""
        return ""

    try:
        resp = session_get(normalized_url)
        soup = BeautifulSoup(resp.text, "html.parser")
        abstract = extract_abstract_from_dom(soup)
        cache[normalized_url] = abstract
        return abstract
    except Exception as exc:  # pragma: no cover - network dependent
        warnings.append(f"{normalized_url}: abstract fetch failed ({exc})")
        cache[normalized_url] = ""
        return ""
...
def dedupe_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if key not in deduped:
            item["abstract"] = clean_placeholder(item.get("abstract"))
            item["journal_or_publisher"] = clean_placeholder(item.get("journal_or_publisher"))
            deduped[key] = item
            continue
        ...
        if not clean_placeholder(existing.get("journal_or_publisher")) and clean_placeholder(item.get("journal_or_publisher")):
            existing["journal_or_publisher"] = clean_placeholder(item["journal_or_publisher"])
        if not clean_placeholder(existing.get("abstract")) and clean_placeholder(item.get("abstract")):
            existing["abstract"] = clean_placeholder(item["abstract"])
...
def enrich_items_with_abstracts(items: list[dict[str, Any]], warnings: list[str]) -> None:
    cache: dict[str, str] = {}
    fetched = 0
    for item in items:
        current = clean_placeholder(item.get("abstract"))
        if current:
            item["abstract"] = current
            continue
        ...
        abstract = fetch_publication_abstract(air_url, cache, warnings)
        item["abstract"] = abstract
        if abstract:
            fetched += 1

    print(f"Fetched {fetched} publication abstracts")
...
def main() -> int:
    ...
    enrich_items_with_abstracts(deduped, warnings)
